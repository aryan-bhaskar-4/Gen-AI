...
const SYSTEM_PROMPT = `
  ...
  Output Format:
  { "step": "INITAL" | "THINK" | "ANALYSE" | "TOOL_REQUEST" | "OUTPUT", "text": "<The Actual Text>" }

`;

const messagesDB = [{ role: "system", content: SYSTEM_PROMPT }];
    ...
    while(true){
        ...
        if (parsedResult.step.toUpperCase() === 'TOOL_REQUEST') {
          ...
          switch(functionName){
            case "getWeatherData" : {
              try {
                const toolrequest = await getWeatherData(input);
...
main("Tell me what's the weather of jaipur?")
